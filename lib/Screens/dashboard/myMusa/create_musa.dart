  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (mounted) {
        ...
        print('passed album---------------------->: ${widget.albumName}');
        // Find and set the matching album from the fetched list
        if (widget.albumName != null && cubit.musaAlbumList.isNotEmpty) {
          final album = cubit.musaAlbumList.firstWhere(
            (a) => a.id == widget.albumName!.id,
          ...
          if (album != null) {
            ...
            print(
                'Selected album---------------------->: ${cubit.selectedAlbum?.title}');
            // Fetch sub-albums for this album
            await cubit.getSubAlbumListApi(
                albumId: album.id.toString(), context: context);

            // Find and set the matching sub-album from the fetched list
            if (widget.subAlbumName != null &&
                cubit.musaSubAlbumList.isNotEmpty) {
              final subAlbum = cubit.musaSubAlbumList.firstWhere(
                (s) => s.id == widget.subAlbumName!.id,
    ...
    cubit.stream.listen((state) {
      if (state is CreateMusaAlbumLoaded && cubit.shouldAutoSelectNewAlbum) {
